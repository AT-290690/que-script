...
fn i32_at(memory: &Memory, store: &Store<ShellStoreData>, addr: i32) -> Result<i32, String> {
...
pub fn run_wat_text(
    ...
    let typ: String = extract_type_from_wat(wat_src).unwrap_or_else(|| "Int".to_string());
    ...
    let mut store = Store::new(
        &engine,
        ShellStoreData::new_with_security(script_cwd, shell_policy).map_err(|e| e.to_string())?
    );
    let instance = linker
...
fn set_argv_strings(
    store: &mut Store<ShellStoreData>,
...
pub fn run_native_shell() -> Result<(), String> {
    ...
    let decoded = run_wat_text(&wat_src, Some(script_cwd), shell_policy, &argv)?;

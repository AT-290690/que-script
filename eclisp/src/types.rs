...
impl TypeEnv {
    ...
    pub fn insert(&mut self, name: String, scheme: TypeScheme) -> Result<(), String> {
        ...
        if name != "_" && name != "." && current.contains_key(&name) {
...
pub fn create_builtin_environment(mut env: TypeEnv) -> (TypeEnv, u64) {
    ...
    #[cfg(feature = "io")]
    {
        for spec in crate::externals::BUILTIN_HOST_EXTERNS {
            let scheme = crate::externals::builtin_host_extern_type_scheme(spec);
            let _ = env.insert(spec.local_name.to_string(), scheme);
        }
    }
    let _ = env.insert(
        "=?".to_string(),

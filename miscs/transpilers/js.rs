...
fn ident(name: &str, idx: usize) -> String {
    ...
    match name {
        ...
        "Dec->Int" => "(a)=>a|0".to_string(),
        "Int->Dec" => "(a)=>a".to_string(),
                ...
                Expression::Word(op) =>
                    match op.as_str() {
                        ...
                        "alter!" => {
                            let name = match &items[1] {
                                Expression::Word(n) => ident(n, 0),
                                _ => panic!("alter! expects a variable name as first argument"),
                            };
                            let value_js = compile_expr_to_js(&items[2]);
                            format!("(({} = {}),0)", name, value_js)
                        }
                        "*" | "*#" | "*." =>
                            format!(
                                "({} * {})",
                        ...
                        "as" | "char" | "Int->Dec" => format!("{}", compile_expr_to_js(&items[1])),
                        "Dec->Int" => format!("({} | 0)", compile_expr_to_js(&items[1])),
